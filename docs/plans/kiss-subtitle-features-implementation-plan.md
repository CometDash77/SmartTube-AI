...
日期：2026-09-20，Asia/Hong_Kong。**状态：K0–K3、K6、K4/K5（规则断句）均已实现，第 1、2 轮候选都通过设备验收；两项外部前提（真实 Key 付费调用、项目签名 Secrets）与部分设备边界交互仍未验证；逐项状态见 §8，验收结果见 §9，下一阶段见 §10，证据见 [实施状态](evidence/kiss-implementation-status-2026-09-20.md)。**
...
| K7 CI/候选/验收 | **第 1、2 轮均已完成并通过设备验收** | 第 2 轮候选 `stbeta-32.53-nightly-27-27-debug`（SHA `3eb560b0`，run 35515384600，68 suites / 558 tests / 0 failures）含规则断句与自查修复，已通过用户验收；第 1 轮候选 `stbeta-32.53-nightly-25-25-debug`（SHA `6c70e370`，run 35510576265，67 suites / 529 tests / 0 failures）已交付并通过用户测试（反馈与边界见 §9）。下一阶段见 §10 |

## 9. 第 1 轮人工验收范围（已实现功能）

**验收结果（2026-09-20，用户反馈）**：**第 1 轮（nightly-25）用户确认“测试没问题”；第 2 轮（nightly-27，含规则断句与自查修复）用户确认验收通过。**记录边界：用户没有逐条说明通过项，因此不写成“§7 清单全部通过”，也不代表真实 Key 付费调用、项目签名/覆盖升级、API 17–22 设备、真实 Keystore/备份导出已经验证。逐项状态与仍缺项见[实施状态](evidence/kiss-implementation-status-2026-09-20.md)，行为基线与下一阶段见当天[进度记录](../../development/2026-09-20.md)。
...
1. ~~**K4/K5 规则断句整条链路**~~：**已完成并通过设备验收**（nightly-27）。（`SubtitleRuleSegmenter` + 派生时间轴 + 显示原子替换 + 有界 100ms 显示检查 + `segmented-*.srt` 导出；细节与仍缺项见当天记录）。原计划措辞保留如下：`SubtitleRuleSegmenter`（保守 native 边界合并、稳定来源映射、局部回退与 `LONG_UNSPLIT` 计数）、派生时间轴（`segmentId / memberItemIds / startUs / endUs / sourceText / ruleVersion`）、planner 选择 raw/derived、单一显示入口的原子替换（派生原文与译文同帧）、边界驱动显示更新（不以 1s 预取 tick 冒充）、导出 `segmented-original/translated/bilingual.srt`。在该链路完成前，菜单里的断句开关不得表现为已生效（隐藏或明确标注，随 K4/K5 一并解决）。
2. **需要用户授权 / 外部前提的两项（当前阻塞在该前提上，不是阻塞在代码）**：真实 Key 的一次最小付费调用（用户授权 + 自备 Key）；配置 4 个签名 Secrets（`SIGNING_KEY / KEY_STORE_PASSWORD / ALIAS / KEY_PASSWORD`）→ `release_mode=true` 产出项目签名 RC，使候选可覆盖升级。
3. **设备边界交互**：权限拒绝、空间不足、遥控器焦点细节与首译延迟的实测记录（可与上面两项合并成一次设备轮次）。
4. **改进类（不依赖外部前提，可先做）**：诊断增量（重翻次数、上下文档位与摘要结果码、派生段数/回退计数、各操作耗时与请求数）；通知文案与提示时长微调；缓存与窗口参数按实测调整；可选把派生句显示从有界 100ms 检查改为计划首选的**下一边界单次唤醒**（当前实现已被设备接受，属可选优化）。
5. **新功能**：待用户指定方向（例如其它 Kiss 能力移植、导出增强或服务侧能力），按同样流程：实现 → 本地门禁 → GitHub-only 候选 → 设备验收。
