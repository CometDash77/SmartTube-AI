...
**2026-09-20 T13 验收后更新：§17 定义近期 N1–N4，§18 将执行链延伸到最终成品 N5–N9；§18 的完整验收口径优先于历史断点。§13–16 保留历史基线。原文导出与 GitHub 候选交付已经完成相应验证，不再按旧断点重做。详见 [T13 阶段报告](evidence/subtitle-t13-progress-report-2026-09-20.md)。**
...
- [阶段报告](evidence/subtitle-t13-progress-report-2026-09-20.md)记录：候选 `stbeta-32.53-nightly-9-9-debug` / `e3512c73` / run `35485905205` 已通过必需字幕测试、lint、组装和签名校验；TCL / Android 11 在 AI 关闭、无 Key 时成功导出 ASR 原文 520 条。此处引用历史证据，本轮没有重新运行 CI 或设备检查；不推广为所有字幕来源均已通过。
- T13 是“核心原文路径通过、其余验收待补”，不是全功能完成。全模块历史两条 ScreensaverManagerTest 失败、lint registry 覆盖限制继续单列，不能写成全库全绿。
- §15 的工作流缺口清单是改造前基线；§16 的“push 不触发”已被阶段报告中后续 push 成功运行的记录取代。未来以实际 ref/run 核查，不重复搭建现有流水线。
- 报告记录同一视频三次快照请求/成功，说明重复工作；现有日志不足以单独证明每次的源身份完全相同。实施前用受控事件测试固定同源场景，不能仅凭计数认定全部来源相同。
- 当前 `PlaybackPresenter.requestAiSubtitleTimeline()` 复用 `mAiSnapshotCancelled`，新请求将其重置；worker 和主线程回调均依赖它。将“旧请求恢复有效后安装过期时间轴/覆盖诊断状态”作为待复现的正确性风险，与去重一起处理，不能只加延时防抖。
- `PlayerUIController` 已有 `SimpleEditDialog.showPassword` 的 Key 输入入口。报告所说“不方便输入”应落到菜单可达性、命名、焦点、保存后生效上；不是从零实现 T08/T09。当前保存回调直接 `keyStore.save()` 后请求时间轴，需验证它是否通知活动配置/解除鉴权停机，并核查 API 17–22 内存 Key store 实例一致性。

### 17.2 用户故事与边界

1. 作为观看者，我希望连续的字幕加载事件只准备一次同源时间轴，以免重复下载影响播放。
2. 作为观看者，我希望换视频、换轨或退出后的旧结果不影响当前字幕和诊断，以便导出内容始终属于当前视频。
3. 作为观看者，我希望能用遥控器找到并完成 Key 的输入、替换、取消和清除，以便使用翻译且不泄漏凭据。
4. 作为观看者，我希望 Key 保存失败时得到可操作反馈，保存成功后无需重启即可按当前 AI 开关状态使用配置。
5. 作为观看者，我希望翻译完成、部分完成、失败或中断时都能导出可播放的字幕和准确状态。
6. 作为观看者，我希望无权限或空间不足时知道如何处理，导出期间仍可正常播放和操作。
7. 作为测试者，我希望每个验收包明确版本、签名与升级条件，以免装错包或丢失数据。

继续复用既有时间轴、缓存、Key store、字幕菜单及导出管线。此次计划不增加手机扫码/局域网凭据服务、云同步、新供应商框架、无限译文缓存或逐条 HTTP 原因数据库；不改变原文回退和 AI 关闭仍可导出的契约。具体 HTTP 失败原因逐条持久化暂缓，`FAILED` 不等同于某个特定 HTTP 错误。

### 17.3 N1 — 请求身份隔离与同源去重（首个实施任务）

- **范围：**`PlaybackPresenter` 的快照调度、事件调用点及必要的 `AiSubtitleSessionBinder` 接口；以现有 source identity 语义为基础，区分视频/媒体源代次与轨道身份。共享模块修改前先解析实际 checkout；默认无需修改子模块。
- **实现：**每次请求持有独立取消状态与不可变请求身份；同一有效来源已有成功时间轴或在飞请求时复用。真实换源、换视频、关闭原生字幕、release 必须失效旧请求；同源重复事件不能先无条件取消再重建。AI 关闭仍允许原文准备，seek 对翻译批次的失效契约保留；是否重取时间轴以源有效性和实际时间轴状态判断。
- **回写：**回到主线程后同时验证请求身份、当前来源与 host 生命周期；过期成功、失败和 finally 均不能安装时间轴、覆盖当前诊断或清除新请求的在飞状态。失败终态释放自身请求占用，后续明确事件可重试；不增加无界自动重试。
- **回归：**用可控制完成次序的 fake fetcher/现有测试 seam 驱动同源连续事件、A→B 后 A 晚成功/晚失败、释放后回调、失败后重试、AI 关闭和 seek。优先在 Presenter 编排边界测试；若现有 seam 不足，只提取最小可测试调度单元并覆盖生产接线。
- **验收：**同一有效来源的一轮事件仅一次 fetch；旧请求不能污染 B；无 Key/AI 关闭原文导出仍通过。保留 `AiSubtitleSessionBinderTest` 的先安装时间轴及字幕恢复重画契约，并回归 `SubtitleSeekCancellationTest`，不得再次以 `refreshSource()` 无条件清空时间轴解决问题。

### 17.4 N2 — 打通已有 Key 配置流程（N1 后）

- **范围：**`PlayerUIController`、`SubtitleAiSettingsController`、`SubtitleKeyStores` 及三套现有字符串。先核查遥控器到“AI 翻译设置”的入口和焦点路径，再修正文案/保存行为；默认复用系统输入法与现有遮蔽输入，不新增远程输入通道。
- **实现：**由活动 settings/controller 统一保存/替换/清除凭据并通知配置变化，避免 UI 新建 store 与活动服务读取不同实例；保存成功刷新“已配置”状态并解除旧鉴权停机；AI 关闭时不自动启动翻译，取消/保存失败保留原有效状态。无需为 Key 保存重复抓取已有原文时间轴。
- **安全契约：**保留 API 23+ Keystore/no_backup 与 API 17–22 会话内保存；不回填 Key、不记录 Key、不隐式运行付费连接测试。真实 Keystore/备份设备检查与 fake 测试分开记录。
- **验收：**测试首次保存、替换、取消、清除、保存失败、鉴权停机后替换、低 API 实例共享；电视验证焦点/键盘/返回路径和保存即生效。仅当系统输入法确实不能满足输入时，再依据设备反馈单独确定替代方案。

### 17.5 N3 — 译文导出与生命周期验收（N1/N2 后）

- **自动检查先行：**复用 `SubtitleExportBundleTest`、`SubtitleExportSnapshotTest`、`SubtitleExportControllerTest`、`SubtitleEndToEndTest` 与取消测试。fake 成功、部分结果、失败及用户中断应生成条件正确的 `translated.srt`、`bilingual.srt`、`untranslated.srt` 和状态文件；不依赖真实 Key。
- **一致性：**以同一点击快照的 item 状态核对统计，单独核对 frame/cue 输出，不能假定 item 数等于 SRT cue 数。核对缺失译文回退、缓存淘汰说明、换轨/换视频中途导出的快照归属与过期回调隔离。
- **设备：**新候选安装后复验 AI 关闭/无 Key 原文导出，再由用户配置 Key 并在实际使用授权内完成翻译成功、双语、部分完成/关闭 AI 后导出；真实失败难以稳定制造时以 fake 证据覆盖并明确设备缺口，不故意消耗额度或制造封禁。
- **证据：**记录 candidate commit/run/tag、设备/系统、操作、诊断白名单与 ZIP 条目/覆盖统计；不记录 Key 或服务正文。新增字幕正文产物是否入库沿用该次任务的实际授权，历史产物授权不推广至任意视频。
- **完成：**自动检查通过且上述设备场景具有对应证据；设备未取得时标注“自动验证通过、设备待验”，不阻塞已能执行的测试。

### 17.6 N4 — 存储、焦点与交付身份收尾

- **存储/体验：**检查 `SubtitleExportFileStore` / `PlayerUIController` 的权限拒绝、空间不足、同名冲突、回读失败、重复点击和退出后反馈；在可控 fake 下验证错误映射，电视检查公共目录可见性、焦点恢复及播放不中断。Android 11 的成功不能替代旧 Android 写权限分支验收。
- **交付身份独立推进：**沿用既有 GitHub workflow，不重建。缺四个签名 Secrets 时现行 debug 回退仅是标注清楚的测试候选，不代表稳定升级身份；跨 runner 的 debug 证书可能不同。项目签名配置、实际安装证书和 versionCode 策略在下一次声称“可覆盖升级”前落实；不默认卸载或改变包名。
- **产物：**绑定同一 SHA 的测试、lint、APK 元数据、签名摘要和哈希，再给出唯一 prerelease；versionCode 当前记录为 2443，versionName 增长不证明升级兼容。基线失败和 lint 未加载规则须在交付记录保留。
- **依赖：**存储与 fake 检查不依赖真实 Key/签名 Secrets；项目签名发布和设备覆盖安装各自等待所需证据，不能把它们当作全部实现工作的阻塞。

### 17.7 执行与关闭规则

唯一当前下一步：**实施 N1，先建立同源事件和旧结果晚到的确定性回归，再修请求身份与取消/去重接线。** N1 → N2 → N3，N4 的独立检查按所需证据推进。所有任务保留现有非本任务工作区改动。

- Gradle 测试、lint、编译、组装及签名验证继续在 GitHub Actions，使用 checked-in wrapper。必需 `exoplayer.other` 测试范围保持；如果新增 Presenter/UI 测试落在该包外，必须将其加入必需 CI 门禁，不能只落入 informational 全模块任务。JDK 11 测试 / JDK 17 lint 构建按现有兼容约束执行。
- 本轮是有界源码定位与文档综合，不是全链代码审查；未进行 Jev 调用，没有适合批量判断的歧义集合。实施 N1/N2 的生命周期与凭据流审查时按项目技能组织实际源码证据的原子 audit；服务失败直接人工复核，不能把 Jev 当验收。
- 文档历史里的推送/API/设备授权仅作上下文；本轮请求为计划更新，不触发 commit/push、工作流、付费 API 或安装。后续执行按实际会话授权推进，不额外引入通用审批关卡。
- T13 关闭需要导出形态、存储边界与遥控器/播放体验的实际验收或明确接受的剩余限制。整个 AI 字幕功能的真实服务、Keystore/备份与其他来源覆盖继续独立追踪，不能用一个 ASR 成功样本概括全部完成。

## 18. 从当前版本一路到最终成品

用户已明确要求计划覆盖最终成品；因此 N1–N4 只是前半程，不能在导出验收或发出 debug 候选后终止。以下是同一实施计划的后半程，不另开待定规划任务。当前仍为计划更新，不代表已经实施或发布。

### 18.1 最终成品定义

交付一个可在声明支持的 Android TV 上安装、配置、日常观看和持续升级的 SmartTube-AI 版本：用户用遥控器选字幕、配置 DeepSeek、开启翻译并切换三种显示模式；网络异常时原文持续，换源与 seek 无过期回写；可导出原文/译文/双语/未译字幕和本地诊断；Key 安全保存且可清除；附实际验收证据、使用说明和升级说明。

沿用 §1 的首版支持边界：可取得完整文本字幕的 VOD（手工字幕、ASR、DASH、SABR、合并源及适用回放）。直播滚动/位图/外部播放器等明确降级，不为“成品”无限扩展功能。对宣称支持的路径必须有证据；样本尚缺时保留未完成，不能静默删除支持承诺。

补充用户故事：

8. 作为观看者，我希望配置服务地址、模型、目标语言和表达指令，并能手动测试连接，以便知道配置是否可用。
9. 作为观看者，我希望错误提示区分来源、网络和鉴权问题，并能按提示恢复，以免重复付费请求或盲目重填 Key。
10. 作为观看者，我希望长时间播放、seek、换轨和退出都保持原字幕体验，以便可靠地日常使用。
11. 作为观看者，我希望重启、备份恢复和版本升级后的配置行为清楚且安全，以免丢失设置或泄漏 Key。

### 18.2 完整执行顺序与检查点

| 阶段 | 工作与依赖 | 出口成果 |
| --- | --- | --- |
| N1 | 请求身份、取消与同源去重，立即开始 | 确定性回归通过，旧请求不污染当前来源 |
| N2 | 修复现有 Key 输入/保存链路，承接 N1 | 可用且安全的基本凭据流程 |
| N3 | N1/N2 后验证各导出形态，fake 部分无需等设备 | T13 译文/中断/失败证据 |
| N4 | 存储/焦点检查；提前准备稳定签名与升级身份 | 导出边界关闭，签名与升级缺口明确 |
| N5 | N2 后补完整配置、连接测试、错误恢复 | 用户可独立完成配置与恢复操作 |
| N6 | N1/N5 后完整来源与真实翻译联调 | 支持范围矩阵和真实服务验收 |
| N7 | N3/N4/N6 后兼容性、安全、性能与完整回归 | 产品质量验收报告，发布阻塞清零 |
| N8 | N7 通过且 N4 稳定签名落实后制作最终 RC | 同一提交的稳定签名候选与升级证据 |
| N9 | 对 N8 原样产物最终验收并交付 | 成品 APK、校验值、文档、发布/回退记录 |

这是依赖关系，不要求等待全部外部条件再做下一项。N3 的真机反馈等待期间可推进 N5；签名材料等待期间可推进 N6/N7 自动检查。默认沿一条实施链工作，不自动增加多 agent。每个检查点失败返回对应任务修复并重跑受影响检查；既有通过证据仅在代码/环境仍适用时复用。

### 18.3 N5 — 完整设置与故障恢复

- **对应原计划：**补齐 T08/T09 和 §5–7 的用户路径，不根据“controller 有方法”认定菜单已接完。盘点现有 `setEndpointBaseUrl`、`setModel`、`setInstruction`、恢复默认、连接测试与重试能力；已有可靠实现直接复用。
- **实现：**现有 AI 设置菜单提供地址、模型、目标语言、表达指令/恢复默认、Key 配置状态/清除、手动连接测试。测试只发送最小合成内容，后台可取消，反馈分类结果，不自动发送正在观看的字幕；地址 origin 变更解除旧 Key 绑定并提示重新配置。
- **故障恢复：**无轨/不支持源、快照失败、网络超时、401/403、余额不足、429/冷却、协议错误分别映射可操作提示；复用现有重试预算和冷却。明确重试入口不得绕过并发、鉴权停机或导致重复计费请求；配置修正后恢复，状态与实际服务一致。
- **验收：**从无配置开始，用遥控器完成输入→连接测试→启用→改语言/模式→错误恢复→清除→取消返回。fake 覆盖各错误分类、取消和跨 origin 不发旧 Key；三套字符串齐全且不显示原始响应。原文/仅译文/双语切换只重画，不重取源或重复翻译。

### 18.4 N6 — 完整播放链与真实服务联调

- **来源矩阵：**按 §1/§11 逐项登记手工字幕、ASR、DASH、SABR、DASH+HLS 合并源和适用回放，记录来源类型、声明 MIME、解码结果、轨道身份和字幕对齐。复用 T13 的 ASR 成功证据，但增加真实翻译验证；无样本路径不能标通过。
- **播放器行为：**播放/暂停/恢复、连续 seek、换视频/轨道/语言、关闭原生字幕、关闭 AI、退出/重建、字幕重新显示、缓存淘汰后回退；核对出现/清屏时间、原生样式与遥控器焦点，所有模式维持单一显示入口。
- **真实 DeepSeek：**由用户配置凭据且实际调用授权成立后，验证当前 endpoint/model 的协议兼容、成功译文、首条译文耗时、预取命中和请求取消；受控错误仍以 fake 补齐。不得因 fake 通过宣称真实服务通过；不把真实 Key 写进测试参数、记录或 Jev 输入。
- **验收：**声明支持来源都能完成选轨→快照→批次→译文显示→导出；不支持来源有明确降级且原生字幕正常。记录请求次数/最大并发/回退/缓存峰值和设备网络条件，不承诺未测的固定延迟或服务 p95。

### 18.5 N7 — 兼容、安全与质量收敛

- **兼容性：**覆盖 API 17–22 内存凭据与 API 23+ Keystore 分支；对生产承诺的最低 API 做实际运行或等价设备检查，不能只用编译代替。覆盖代表性 Android TV 存储路径和发布 ABI；缺设备列为具体未验项，独立自动检查继续。
- **安全：**fake marker 验证日志、诊断、应用备份、系统备份/迁移路径不泄漏；真机验证 Key 重启读取、清除、Keystore 失效后受控恢复及换设备不可用状态；核查 origin 绑定、重定向和专用 HTTP client。诊断白名单与有正文的字幕导出分开。
- **性能与资源：**至少一段 30 分钟连续播放并穿插 seek/换轨/AI 开关/导出，记录设备与媒体时长、请求最大并发≤1、队列/缓存不超 §6 上限、退出后 worker/请求释放；无可归因的新崩溃、ANR、播放中断或持续增长。短测结果不冒充长期稳定性。
- **回归门禁：**受影响测试、完整 common 测试、相关播放器/来源测试、两模块 lint 和生产变体构建。调查并修复仍存在的两条 ScreensaverManagerTest 基线失败，查清 lint registry 未加载原因并恢复规则；最终不能永久靠 informational/continue-on-error 隐藏质量债务。若确属工具不可用的限制，必须记录具体覆盖缺口并取得明确接受，不能自动视为已通过。
- **review：**按实际最终 diff 复查来源→时间轴→调度→显示→导出、凭据/备份路径和 off-path，依据项目要求执行适合的 Jev audit 并人工核对反例；修复发现的问题，不把审查结果替代运行证据。
- **契约更新：**§11 历史“AI 从未启用不创建 AI worker”已被 T13 的独立原文快照要求部分取代：AI 关闭可以创建原文抓取 worker，但不能创建翻译请求或付费调用；测试必须区分这两类工作。
- **完成：**无未解决的核心正确性、凭据泄漏、安装/升级或崩溃阻塞；声明范围的自动/设备证据齐全，其他限制准确列明。缺真实设备/服务证据时仍是待验产品，不称成品。

### 18.6 N8 — 稳定签名 RC 与升级验证

- **前置：**N7 质量门禁通过；N4 落实持久项目签名及 Secrets，确定最终包名和已安装证书；按实际已发布/已安装 versionCode 制定单调增长规则，防止 nightly 消耗未来版本号空间。不能凭固定 2443 或 versionName 推断升级成功。
- **CI：**保留 debug 回退供开发，但“最终 RC/正式发布”路径必须缺签名即停止，不退回 debug。必要时增加明确的发布模式与对应 workflow 检查；不将历史 debug 候选升级成最终产物。
- **验证：**GitHub 对固定 SHA 运行必需门禁、release 组装、所有待发布 ABI/Universal 的包元数据与 `apksigner verify`、证书和 SHA-256；不把签名文件作为 artifact。输出唯一 RC prerelease 与证据清单。
- **升级：**在同签名受支持旧版本上覆盖升级，核对配置、Key/会话规则、字幕与导出；另做全新安装。已有 debug/官方不同签名版本分别说明迁移限制，不默认卸载。为失败升级准备可执行的恢复说明，明确 Android 降级/数据恢复限制，不虚构无损回滚。
- **完成：**RC 的下载资产、验证资产和待最终交付资产字节一致，实际升级测试通过；修代码即新 SHA/新 RC，再跑受影响门禁。

### 18.7 N9 — 最终验收与成品交付

1. 对 N8 原样 RC 做最终 TV 验收：首次配置、连接测试、真实翻译三模式、seek/换轨/退出、故障降级、各形态导出、Key 清除与升级。将结果逐项链接到 §18.8，不以“用户未报告异常”代替验证。
2. 完成用户说明：安装与升级、长按字幕菜单入口、Key 输入与服务费用/数据发送说明、目标语言和表达指令、错误恢复、文件位置/条目/缓存覆盖限制、支持范围、已知限制和诊断反馈方法。
3. 汇总最终版本、commit/run/tag、APK 下载项/ABI/minSdk/包名/versionCode/versionName、哈希与证书、测试与设备矩阵、剩余非阻塞限制、升级/恢复步骤；交付可安装文件，不能只给源代码或日志。
4. 在实际正式发布授权覆盖时，用已验收的同一产物发布最终 release；如果该外发步骤未获授权，先完成 RC、全部证据和 release 文案，再仅对最后发布动作取得授权。此时准确标为“成品已验收、待发布”，不声称已发布。不得在计划阶段索要签名或真实 Key。
5. 发布后核对远端资产哈希与链接、版本说明和升级身份，更新开发记录/实现状态/阶段报告的当前摘要；保留旧证据。最终交付时没有被遗忘的待实现核心功能。

### 18.8 最终完成清单（全部通过才称成品完成）

| 能力 | 必需证据 | 责任阶段 |
| --- | --- | --- |
| 时间轴不重复、不串源、原生字幕回归 | 事件/晚回调自动测试 + TV 生命周期验收 | N1/N6 |
| 配置、Key、连接测试和恢复可操作 | fake 安全/错误测试 + TV 输入/Keystore/备份验证 | N2/N5/N7 |
| 承诺范围内真实翻译与三模式 | 来源矩阵 + 真实服务与 TV 播放记录 | N6 |
| 导出完整、部分、失败及诊断 | ZIP/状态一致性测试 + TV 产物与存储边界 | N3/N4 |
| 兼容、性能、安全与回归 | CI 报告、lint 覆盖、低 API/长时播放记录、最终 review | N7 |
| 可安装、可持续升级 | 稳定证书/版本策略 + 全新安装与覆盖升级证据 | N8 |
...
> **状态更正（2026-09-20 12:55）**：本节 19.1 记录的是"代码与自动测试完成"，**不等于设备验收通过**。
> 第二轮设备验收（`nightly-18`，12:47–12:49）**未通过**，原因与缺陷清单见 §20 与调试报告
> `docs/plans/evidence/subtitle-ai-tv-acceptance-round2-debug-2026-09-20.md`。特别是 N5 的"菜单可用"与
> T13 的"无 Key/无 AI 也能导出原文"两条，**已被设备证据部分推翻**（前者标签不可识别，后者要求先选中文本字幕轨）。
...
> 上述"已完成"的**设备侧**结论已被第二轮验收修正：见 §20。
## 20. 第二轮设备验收：未通过（2026-09-20 12:47–12:49，nightly-18）——待审查后决定修复

> **复核更新：本节保留为历史诊断，当前结论与实施顺序以 §21 为准。** 失败计数、Key 历史、签名清数据和 N1 守卫等价的表述已修正；不要把以下待审建议作为已证实根因。

本节只记录**证据化的结论**与**待审查的修复清单**。完整调试报告：
`docs/plans/evidence/subtitle-ai-tv-acceptance-round2-debug-2026-09-20.md`；
原始诊断报告已按字节入库到 `docs/plans/evidence/device-logs/2026-09-20/`。

### 20.1 结论

- **验收不通过**：用户报告的"不开 AI 与开了 AI 都导不出字幕""找不到填 API Key 的地方""AI 区功能测不了"全部成立。
- **导出失败的直接原因（已证实）**：导出时刻**没有任何"已绑定"的字幕来源**（`sourceBound=false`、`sourceType=unknown`），
  整个会话**从未发起时间轴抓取**（`snapshotStatus=NOT_REQUESTED`，0 条 `TIMELINE_REQUESTED`，事件环未满）；
  因此无原文可导出，导出按设计拒绝且未产生空文件。
- **与 AI 开关无关**：`AI_ENABLED` 前后失败码相同，打开后仍无抓取事件。
- **写文件能力正常**：同会话 `EXPORT_DIAGNOSTICS_OK` 两次，剩余空间约 19.8 GB。
- **不是 N1 请求身份改动的证据性回归**：与旧实现的守卫等价（新增的 `sourceKey == null` 与 `source == null` 同源同真），
  无任何证据显示"N1 之后抓取变少"。
- **现有诊断无法区分**四种可能（没选轨 / 选轨但来源无法绑定 / 请求被内部守卫拦下 / 未走播放器界面），
  这是本轮必须一并修的**元问题**。
...
| 编号 | 缺陷 | 证据 | 建议修复 |
| --- | --- | --- | --- |
| P0-1 | 无绑定字幕轨时，失败提示"请等待数秒后重试"把用户引向错误方向 | 29 次 `NO_TIMELINE`，`sourceBound=false` | 按 `sourceStatus` 分三种可操作提示（未选轨 / 来源不可识别 / 时间轴未就绪） |
| P0-2 | 诊断缺"是否选中字幕轨 / 来源绑定状态 / 抓取为何被跳过" | `NOT_REQUESTED` 无法定因 | 新增白名单字段 `subtitlesSelected`、`sourceStatus`、`timelineRequests`、`timelineInstalls`、`timelineSkips` 与跳过原因事件码 |
| P0-3 | Key 入口不可识别：标签与输入框 hint 都是"AI 翻译设置"，全程无 "Key" 字样；保存无成功反馈；菜单不显示是否已配置（`ai_subtitle_key_configured`/`ai_subtitle_key_missing` 引用次数为 0） | 用户 3 条抱怨 + 字符串引用计数 | 入口改为 "API Key（DeepSeek）：未配置/已配置"、hint 说明用途、保存成功 toast、状态显示 |
| P1-1 | AI 关闭时状态行显示"翻译中"（`AI_OFF` 落到 `default` 分支） | 代码 `aiSubtitleStatusResId` | 新增"AI 翻译未开启（本视频）"并显式映射 |
| P1-2 | AI 区无使用顺序提示；无轨时不提示，只有导出失败后才暴露 | 用户"测不了" | 区首加"① 选字幕轨 ② 输入 API Key ③ 打开开关"，并把 `subtitlesSelected` 接入菜单状态 |
| P1-3 | 每个候选的 debug 签名都不同 → 跨签名必须卸载重装 → 数据与 CC 偏好被清空，放大 P0-1 | 本机实测两次候选证书不同；日志中设置全为默认 | 先落实 4 个签名 Secrets 产出项目签名候选（N8），或同一次安装内验完全部项目 |
| P2 | 无绑定轨时导出按钮仍可点且只有失败对话框；`ai_subtitle_export_no_timeline` 一条文案承担三种含义 | 29 次失败 | 即时提示 + 文案拆分为三条 |

### 20.3 下一步（按依赖）

1. 由审查者确认 20.2 的优先级与范围（特别是 P0-1 是否需要扩大到"允许从屏幕上已显示的 cue 生成原文"这一契约变更）。
2. 实施 P0-1/P0-2（含单元测试）→ CI 绿 → 出新候选。
3. 实施 P0-3/P1-1/P1-2 → CI 绿 → 出新候选（可与上一步合并为一个候选，取决于审查意见）。
4. P1-3 签名 Secrets 到位后产出**项目签名**候选，再安排第三轮设备验收（按调试报告第 7 节的最短路径）。
5. 第三轮验收失败时，新诊断字段应能直接定位原因，不再需要人工往返推断。

### 20.4 本轮不做

- 不实施 20.2 的任何修复（等审查者决定）；不改契约、不改导出行为、不动 §14/§15 的交付约定。
- 不重跑 CI、不发布**用于验收**的新候选（本次文档/工作流提交会按现有工作流产出一个同代码候选，不作为验收版本）、不改子模块。

## 21. 独立复核后的更新计划（2026-09-20，待实施）

用户要求本轮只交付复核和实施计划，由用户转交另一位 agent；未修改应用代码。
详细证据：[第二轮独立复核](evidence/subtitle-round2-independent-review-2026-09-20.md)。
执行任务与测试/设备门槛：[第三轮实施计划](subtitle-round3-implementation-plan.md)。

### 21.1 当前结论

- 维持设备验收不通过。直接失败是没有可导出时间轴；三个诊断时刻无绑定来源，未见抓取启动事件。没选轨、绑定失败、守卫/回调不同步仍不能定因，N1 回归既未证实也未排除。
- 重算日志：三份失败报告是同一序列的累计快照，最新报告为 **12 次字幕失败、2 次诊断成功**，不是 29 次失败。
- 用户在本轮明确确认“没有地方输入也没有输入过key”。据此接受未输入事实；源码有密码框不代表真实设备可用，入口/焦点/输入/保存必须一起验收。
- 接受 Key 标签与持久保存反馈缺陷、AI_OFF 文案错误。新增确认：菜单把格式包装对象存在当成绑定成功；绑定状态需要刷新；导出异步结果的解释应来自点击时快照。
- 不把旧候选证书差异与默认值推导为本轮确实卸载清数据。稳定签名是持续升级前提，不阻塞本轮代码修复。
- Jev live audit 1 次/6 项，输入输出和人工裁决已留档；其中菜单绑定判断的低置信支持被源码反例推翻。未以 Jev 替代测试或设备验收。

### 21.2 实施顺序与范围

1. R1：复用现有 host/快照/协调器，统一 selected、bound、readiness，固定请求结果、计数、最近原因及隐私白名单。
2. R2：从点击时快照产生导出失败原因；无轨/未知来源/未就绪/失败终态分别给恢复动作；保留导出按钮与诊断可达。
3. R3：可发现、可输入的 API Key 入口与反馈；显式 AI_OFF；分别说明原文无需 Key 与 AI 翻译步骤。原 P1-1 升为本轮必修，P2 并入 R2。
4. R4：回归选轨→来源身份刷新→原文准备；证明漏触发后才最小修复，不回退 N1、不加轮询/第二下载器。
5. R5/R6：整合为一个修复候选，按现有 CI 必需门禁验证，再做第三轮实际 TV 验收。项目签名/覆盖升级与真实 Key 调用分别作为依赖出口，不阻塞其他工作。

不改“导出已有完整时间轴”契约，不从屏幕 cue 拼造原文，不搬迁整个设置中心；用户暂无须对这些常规范围选择再次审批。
