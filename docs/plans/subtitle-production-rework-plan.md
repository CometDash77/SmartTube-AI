...
> **用户澄清后的优先级更正**：本次“失败”具体是点击保存 API Key 后，无论填写什么都显示保存失败。先修复 `SubtitleKeyCipher` 的外部 IV 与 AndroidKeyStore 随机加密策略冲突，以及 Keystore 不可用时未进入内存回退；CI/电视验证尚待完成。下文“失败操作未知”是规划时的历史信息，现已被此澄清替代，不能继续调查成字幕导出失败。整体重改范围保留。
